document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    form.addEventListener('submit', e => {
      ...
      const v = (id, test) => { const el = form.querySelector('#' + id); if (!test(el.value.trim())) { el.closest('.form-group').classList.add('error'); ok = false; } };
      v('name', s => s.length >= 2);
      v('email', s => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s));
      v('phone', s => /^[\d\s+\-()]{10,15}$/.test(s));
      v('message', s => s.length >= 10);
      if (ok) { form.style.display = 'none'; document.querySelector('.form-success').style.display = 'block'; }
    });
  }
});
